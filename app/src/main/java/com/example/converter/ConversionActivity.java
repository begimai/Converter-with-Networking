...
public class ConversionActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		firstValueEditText =
			(EditText) findViewById(R.id.firstValueEditText);
		secondValueEditText =
			(EditText) findViewById(R.id.secondValueEditText);

		firstUnitSpinner =
			(Spinner) findViewById(R.id.firstUnitSpinner);
		secondUnitSpinner =
			(Spinner) findViewById(R.id.secondUnitSpinner);

		int resourceId =
            getIntent().getExtras().getInt(MainActivity.UNITS_INTENT_EXTRA);
        previousUserValues =
			getSharedPreferences(PREFERENCE_FILE_SUFFIX + resourceId, 0);
	...
	private void loadConversionData() {
		int resourceId =
			getIntent().getExtras().getInt(MainActivity.FACTORS_INTENT_EXTRA);
	...
	private void recalculateResult(
		...
		int fromSelectedUnitPosition =
            fromUnitSpinner.getSelectedItemPosition();
		int toSelectedUnitPosition =
            toUnitSpinner.getSelectedItemPosition();
		...
		double fromValue =
            conversionFactors.getFloat(fromSelectedUnitPosition, 1.0f);
		double toValue =
            conversionFactors.getFloat(toSelectedUnitPosition, 1.0f);
